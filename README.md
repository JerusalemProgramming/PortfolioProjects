...
Naturally each "Item_" could possibly be sold to only one (1) "Client_" if/when that Item in inventory becomes sold.  It would therefore be necessary to connect "Client_" info to each "Item_" that becomes sold through the "Item_Sold2WhichClient" field which would be updated if/when the Item is sold with specific "Client_" info, specifically "Client_ContactDetails" object that contains the following five (5) specific Key:Value details about each "Client_":  1.) "Client_NameLast", 2.) "Client_NameFirst", 3.) "Client_Address", 4.) "Client_Telephone", 5.) "Client_Email".
